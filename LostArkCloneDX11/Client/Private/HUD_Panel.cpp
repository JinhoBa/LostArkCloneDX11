...
CHUD_Panel::CHUD_Panel(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CUIPanel{ pDevice, pContext }
{
}

CHUD_Panel::CHUD_Panel(const CHUD_Panel& Prototype)
	: CUIPanel{ Prototype }
{
}

HRESULT CHUD_Panel::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CHUD_Panel::Initialize(void* pArg)
{
...
void CHUD_Panel::Priority_Update(_float fTimeDelta)
{
}

void CHUD_Panel::Update(_float fTimeDelta)
{
	Update_Position();
}
...
HRESULT CHUD_Panel::Add_Components()
{
	/*Texture*/
...
HRESULT CHUD_Panel::Ready_Layer(const _wstring& strLayerTag)
{
	UIOBJECT_DESC Desc = {};
	...
	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_TO_INT(LEVEL::GAMEPLAY), TEXT("Prototype_GameObject_BuffUI"),
		ENUM_TO_INT(LEVEL::GAMEPLAY), strLayerTag, &Desc)))
		return E_FAIL;
...
HRESULT CHUD_Panel::Add_QuickSlots(const _wstring& strLayerTag)
{
	_uint iKeyIndex = {};
	_byte SlotKeys[] = {
...
CHUD_Panel* CHUD_Panel::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CHUD_Panel* pInstance = new CHUD_Panel(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Create : CHUD_Panel");
		return nullptr;
	}
	return pInstance;
}

CGameObject* CHUD_Panel::Clone(void* pArg)
{
	CGameObject* pInstance = new CHUD_Panel(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Clone : CHUD_Panel");
		return nullptr;
	}
	return pInstance;
}

void CHUD_Panel::Free()
{
	__super::Free();
}
