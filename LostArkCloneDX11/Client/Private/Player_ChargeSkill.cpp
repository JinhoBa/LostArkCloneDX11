...
{
	CHARGESKILL_DESC* m_pSkill_Desc = static_cast<CHARGESKILL_DESC*>(pArg);
	...
	m_fChargeTime = 0.f;
	m_fMaxChargeTime = 2.f;

	if (12 == m_pSkill_Desc->iSkillID)
	{
		m_iAnimStart = 126;
		m_iAnimLoop = 125;
		m_iAnimEnd = 124;
...
void CPlayer_ChargeSkill::Update(_float fTimeDelta)
{
	...
	case Client::CPlayer_ChargeSkill::START:
		if (m_pPlayer->isAnimationFinish())
		{
			...
			m_pPlayer->Set_Animation(m_iAnimLoop, true);
		}
		break;

	case Client::CPlayer_ChargeSkill::LOOP:
		m_pPlayer->TurnToCursor();
		if (!m_pGameInstance->Get_KeyPressing(m_iKey) || m_fMaxChargeTime <= m_fChargeTime)
		{
			if(12 == m_iSkillID)
			{
			...
			else
			{
				...
				m_pPlayer->Set_Animation(m_iAnimLoop, false);
			}
		}
		break;
